...
- Invoke the service by the following template:
  ...
  |parameter|value|
  |:---------|:-----|
  |{accessToken}|is an access token which lasts for one hour or false. When accessToken equaling to false, the query isn't augmented. When the accessToken is invalid (incorrect or expired), the servlet returns the error in json format - { AccessToken: 'Access token is incorrect or expired' } | 
